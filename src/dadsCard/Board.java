...
public class Board {
	...
	/**
	 * Find the number of blocks out of place
	 * 
		...
		if (!manhattened) {
			...
			for (int x = 0; x < N; x++)
				...
				for (int y = 0; y < N; y++) { // y-dimension, traversing cols
												// (j)
					int value = tiles[x][y]; // tiles array contains board
												// elements
					if (value != 0) { // we don't compute MD for element 0
						...
						int targetY = (value - 1) % N; // expected y-coordinate
														// (col)
						int dx = x - targetX; // x-distance to expected
												// coordinate
						int dy = y - targetY; // y-distance to expected
												// coordinate
	...
	public Board twin() {
		...
		while (tiles[randRow][randColumn] == 0
				|| tiles[randRow][randColumn + 1] == 0) {
	...
	private Board swap(int intialRow, int initialColumn, int goalRow,
			int goalColumn) {
